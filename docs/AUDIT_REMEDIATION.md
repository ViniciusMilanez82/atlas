...
Baseline: `d81cece` (Alpha 2). Branch: `impl/contract-v2` (integrada no PR #5). Atualizado em 2026-09-25.

**Cobertura real.** "32 corrigidos" quer dizer 32 rotas reproduzidas e corrigidas. A revisão independente
do PR #5 reabriu rotas diferentes de A3-02, 03, 04, 07, 09, 10, 12, 14, 15, 18 e 26 (R5-01..R5-09); elas
estão marcadas abaixo e detalhadas em `docs/REVIEW_PR5_REMEDIATION.md`. Uma rota corrigida não prova
todas as rotas da mesma classe.
...
| A3-02 | Sentinelas SENSITIVE (memória, anexo, eco da conversa) chegaram ao payload do modelo | `9a79555` classificação até o payload + Egress Guard por provedor/finalidade | `test_a3_02.py` | IMP, INT; **reaberto: R5-01 (correção, resposta, outbox, eco) → `b61fb3b`, `179ea89`** |
| A3-03 | Relatório "por preço" foi despachado após a correção "priorize prazo" (2 escritas) | `d2fad8f` revisões de instrução; broker recusa proposta obsoleta | `test_a3_03.py` | IMP, INT; **reaberto: R5-04 (`finish` obsoleto, correção entre verificação e commit) → `5a0b657`** |
| A3-04 | `stopAll()` → `send()` ignorado com `isSending`; fila única | `d2fad8f` `control.stop` + conexão de controle própria no app | `test_a3_04.py` + `A304RegressionTests.swift` | IMP, INT, MAC-CI (VM com transporte falso); botão real: HW pendente; **reaberto: R5-03 (interpretação publicada após STOP) → `1b40dea`** |
...
| A3-07 | Receita de bolo com fontes inventadas e conta errada PASSOU no verificador | `024d892` critérios derivados do pedido, evidência por critério | `test_a3_07_08.py` | IMP, INT; **reaberto: R5-05 (negação, totais em prosa/tabela/planilha, critério invertido) → `d3ff3ce`; R5-08 (fonte de outro escopo) → `94b9e0c`** |
| A3-08 | `(?i)\bTODO\b` marcava "Todo o trabalho foi concluído" | `024d892` marcadores explícitos apenas | `test_a3_07_08.py` | IMP, UNIT |
| A3-09 | DOCX decodificado como UTF-8: 14 169 caracteres de substituição, valor da tabela ausente | `7aa1c24` extratores por formato (ADR-016) | `test_a3_09_10.py` | IMP, INT; bundle com dependências gerado no CI; **reaberto: R5-06 (PARCIAL como leitura integral) → `d3ff3ce`** |
| A3-10 | `text[:200_000]` e `json.dumps(...)[:20_000]` (estático) + T13 | `7aa1c24` leitura paginada com cobertura; JSON nunca cortado | `test_a3_09_10.py` | IMP, INT; **reaberto: R5-06 (cobertura de extração ≠ de leitura) → `d3ff3ce`** |
...
| A3-12 | Worker via tarefa CREATED com 0 de 3 anexos logo após o commit | `64e9575` publicação atômica | `test_a3_12.py` | IMP, INT; **reaberto: R5-07 (anexo enviado como resposta) → `b81c59d`** |
...
| A3-14 | Status e assunto novo consumidos como resposta à pergunta pendente | `c62e191` alvo explícito; inferência só inequívoca; ambiguidade pergunta qual | `test_a3_13_14_15.py` + `G3RegressionTests` | IMP, INT, MAC-CI (VM); **reaberto: R5-07 (resposta com anexo a outra tarefa) → `b81c59d`** |
| A3-15 | App enviava `delegate || !ids.isEmpty` | `c62e191`, `8a6a5a4` guardar / anexar à tarefa / analisar distintos | `test_a3_13_14_15.py` + `G3RegressionTests` | IMP, INT, MAC-CI (VM); **reaberto: R5-07 (anexar à tarefa em dois commits) → `b81c59d`** |
...
| A3-18 | Correção de 4 KB descartada em histórico saturado; resposta antiga como VERIFIED FACT | `7b0b3bd` itens obrigatórios reservados; histórico como não verificado | `test_a3_18.py` | IMP, INT; **reaberto: R5-02 (pedido integral trocado pelo resumo) → `c793b1c`** |
...
| A3-26 | Proprietário A corrigiu memória do empregado B | `5b9a3e6` verificação central por objeto (14 chamadas cruzadas) | `test_a3_26.py` | IMP, INT; **reaberto: R5-08 (fonte web de outro empregado) → `94b9e0c`** |
...
## Achados da revisão independente do PR #5

R5-01..R5-09 — ver `docs/REVIEW_PR5_REMEDIATION.md` (reprodução, causa, regressão que falha antes e passa
depois, commit, evidência e pendência de cada um).
